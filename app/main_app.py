...
from multimodal.Youtube import PytchatMonitor
...
class AIAssistantApp:
    ...
    def is_screen_analysis_triggered(self, user_utterance: str) -> bool:
        ...
        triggers = self.settings.get('SCREEN_TRIGGER_PHRASES', [])
        ...
        for t in triggers:
            ...
            if t in user_utterance:
                ...
                logger.debug(f"Screen analysis trigger matched: '{t}' in '{user_utterance}'")
    ...
    async def _handle_screen_analysis(self, user_utterance: str) -> str:



        logger.info("Screen analysis triggered by user utterance.")

        try:

            img_bytes = await self.screen_capture.capture()

            if not img_bytes:

                logger.error("Screen capture returned no data.")

                return "画面のキャプチャに失敗しました。"



            # PIL に変換してリサイズ（内部で JPEG バイト列を返す）

            pil_img = self.screen_capture.bytes_to_pil(img_bytes)

            if pil_img is None:

                logger.error("Captured bytes could not be converted to PIL Image.")

                return "キャプチャした画像の処理に失敗しました。"



            resized_bytes = self.screen_capture.resize_image(pil_img, max_width=1024)

            final_bytes = resized_bytes if resized_bytes else img_bytes



            # 保存パス: project_root/data/screen_capture_<timestamp>.jpg
            ...
            filename = f"screen_capture_{ts}.jpg"
            ...
            # --- AI に画像を送り、「感じたこと・気づき・考え」を生成させる ---

            prompt_prefix = (

                "以下の画面を見て、あなたが『感じたこと』『気づいた点』『考えたこと』を自然な会話になるように述べてください。"

            )

            combined_utterance = prompt_prefix + ("\n" + (user_utterance or ""))
            ...
            self.conversation_log.append({'speaker': 'ai', 'utterance': response_text})



            logger.info(f"Saved screen capture to {save_path} and AI responded.")
            ...
            logger.exception(f"画面解析処理中にエラーが発生しました: {e}")

            return "画面のキャプチャ中にエラーが発生しました。"
